...
def process_image(msg):
    ...
    try:
        ...
        data_y.data = [x_obj, y_obj, x_robo, y_robo]
        pub.publish(data_y)
...
def start_node():
    global pub
    rospy.init_node('detect_image_y', anonymous=True)
    rospy.loginfo('detect_img_y node started')
    rospy.Subscriber("/vehicle/camera_y/image", Image, process_image)
    pub = rospy.Publisher('/data_y', Float64MultiArray, queue_size=10)
    rospy.spin()
